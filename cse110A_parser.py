...
class Parser:
    ...
    def parse_for_statement(self) -> List[str]:
        ...
        if self.uf > 1:
            # To parse instructions we use regular expressions and follow the format of the example in the assignment and what we did in LVN
            ...
            parsed_original_assignment = parse_instructions(original_assignment_program)
            parsed_expr = parse_instructions(expr_program)
            parsed_loop_end_assignment = parse_instructions(loop_end_assignment_program)
            parsed_loop_body = parse_instructions(loop_program)

            # we check that the first assignment is a constant assignment to the iteration variable
            if len(parsed_original_assignment) == 2 and parsed_original_assignment[0].get("instr") == "int2vr":
                # identify the iteration variable (lhs of the first assignment statement)
                iteration_variable = parsed_original_assignment[1].get("assigned")
                # we determine that the iteration variable is assigned a constant integer in the assignment statement because the instr was int2vr
                initial_val = int(parsed_original_assignment[0].get("op1"))
            else:
                iteration_variable = None

            # we determine that the condition expression is a less-than expression with the iteration variable on the lhs and an integer constant on the rhs
            if iteration_variable and len(parsed_expr) == 2 and parsed_expr[0].get("instr") == "int2vr" and parsed_expr[1].get("instr") == "lti" and parsed_expr[1].get("op1") == iteration_variable:
                final_val = int(parsed_expr[0].get("op1"))
                num_iterations = final_val - initial_val
                # we check that the unroll factor evenly divides the loop iterations
                condition_check = num_iterations % self.uf == 0
            else:
                condition_check = False

            # we determine if the update assignment expression updates the iteration variable by adding the value 1 (i.e. it is an increment)
            if condition_check and len(parsed_loop_end_assignment) == 3 and parsed_loop_end_assignment[0].get("op1") == "1" and parsed_loop_end_assignment[1].get("instr") == "addi" and parsed_loop_end_assignment[2].get("assigned") == iteration_variable:
                end_assignment_check = True
            else:
                end_assignment_check = False

            # we check if the loop body has any control flow (i.e. branches or labels)
            # and we check if the loop body updates the iteration variable
            if end_assignment_check:
                loop_body_check = all((instr.get("label") is None) and (instr.get("instr") not in ["branch", "beq", "bne", "blt", "bgt", "ble", "bge"]) and instr.get("dst") != iteration_variable for instr in parsed_loop_body)
            else:
                loop_body_check = False

            # If all conditions pass, unroll the loop
            if loop_body_check:
                loop_program = loop_program + (loop_end_assignment_program + loop_program) * (self.uf - 1)
